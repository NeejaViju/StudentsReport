...
# New function for checking file presence
def is_file_present(expected_file, files_in_folder):
    return any(expected_file.lower() == file_in_folder.lower() for file_in_folder in files_in_folder)

# Updated function to validate week folder contents
def validate_week_folder(week_folder_path, expected_files):
    files_in_folder = os.listdir(week_folder_path)
    files_in_folder_stripped = [os.path.splitext(f)[0].strip().lower() for f in files_in_folder]
    present_files = [file for file in expected_files if is_file_present(file, files_in_folder_stripped)]
    missing_files = [file for file in expected_files if not is_file_present(file, files_in_folder_stripped)]
...
for student_id, student_name in student_data.items():
    ...
    if os.path.exists(student_folder_path) and os.path.isdir(student_folder_path) and os.path.exists(week_folder_path) and os.path.isdir(week_folder_path):
        ...
        completion_status = "Completed" if len(present_files) == len(expected_files) else "Pending"
